...
def get_snoop():
    client = Kathara.get_instance().manager.client
...
def start_snoop():
    ...
    args = {
        ...
        "detach": True,
        "labels": {
            "lab_hash": Lab(utils.get_vlab_temp_path()).hash,
            "name": "wire_snoop",
            "app": "kathara",
...
def snoop_port(name):
    docker_link = Kathara.get_instance().manager.docker_link
    try:
        network = docker_link.get_links_api_objects_by_filters(link_name=name).pop()
    except:
